...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("🥴 ചേ നാണകേട്.", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("വീണ്ടും ശ്രമിക്കു 🙂.",show_alert=True)
        return
...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("വീണ്ടും ശ്രമിക്കു 🙂.", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('☺️ ΛᎠᎠ MΞ ϮԾ YԾUᏒ GᏒԾUᎮ 😊', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
            ],[
            ...
            InlineKeyboardButton('☺️ SUBSCᏒIBΞ ☺️', url='https://youtube.com/channel/UCf_dVNrilcT0V2R--HbYpMA')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)      
        await query.message.edit_text(
